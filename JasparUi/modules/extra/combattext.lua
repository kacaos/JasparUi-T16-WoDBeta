...
for i = 1, numf do
	...
	f:SetFont(C.Medias.Font, 20, "OUTLINE")
	f:SetShadowColor(0, 0, 0, false and 1 or 0)
	f:SetShadowOffset(false and 1 or 0, false and -1 or 0)
	...
	if i == 1 then
		...
		if _G.oUF_Player then
			f:SetPoint("BOTTOMLEFT", _G.oUF_Player, "TOPLEFT", -200, 60)
		else
	...
	elseif i == 2 then
		...
		if _G.oUF_Player then
			f:SetPoint("BOTTOMLEFT", _G.oUF_Player, "TOPLEFT", -200, 180)
		else
			f:SetPoint("CENTER", 192, -32)
		end
	elseif i == 3 then
...
local StartConfigmode = function()
	if not InCombatLockdown()then
		for i = 1, #ct.frames do
			...
			f.fs:SetFont(C.Medias.font, 20, "OUTLINE")
			...
			if not C.combattext.scrollable then
				f:SetScript("OnSizeChanged", function(self)
					self:SetMaxLines(self:GetHeight() / 20)
					self:Clear()
				end)
			end
